#!/bin/bash
# Script de backup automatizado para Linux
# Executa backup e verificacao usando Python diretamente
# Uso: ./scripts/backup_task.sh

# Configuracoes
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VENV_PATH="$PROJECT_DIR/.venv/bin/activate"
LOG_DIR="$PROJECT_DIR/logs"
LOG_FILE="${1:-logs/backup_task.log}"
...
exit_with_error() {
    ...
    write_task_log "=== BACKUP AUTOMATIZADO FALHOU ===" "ERROR"
    exit 1
}

# Inicio do script
write_task_log "=== INICIANDO BACKUP AUTOMATIZADO ===" "INFO"
...
# Executar backup
write_task_log "Executando backup..." "INFO"
if python "restic_backup.py" 2>&1 | tee -a "$FULL_LOG_PATH"; then
    write_task_log "Backup concluido com sucesso" "SUCCESS"
else
    exit_with_error "Falha no backup. Codigo de saida: $?"
fi
...
write_task_log "=== BACKUP AUTOMATIZADO CONCLUIDO ===" "SUCCESS"
exit 0
